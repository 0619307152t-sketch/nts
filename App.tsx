...
import { Droplets, Thermometer, Sun, CloudRain, Sprout, Palette, LayoutDashboard, RefreshCw } from 'lucide-react';
...
import { SensorReadings, SystemMode, PumpStatus, WeatherForecast, HistoricalDataPoint, WaterUsageLog, PlantConfig, AIActivityLog, ThemeConfig, ThemeColor } from './types';
import { fetchSheetData, logWaterUsageToSheet } from './services/sheetService';
...
const APP_CONFIG = {
  ...
  appSuffix: "AI",
  subtitle: "Intelligent Irrigation System",
  defaultPlant: {
...
function App() {
  ...
  const [waterUsageData, setWaterUsageData] = useState<WaterUsageLog[]>([]);
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      if (data.history && data.history.length > 0) {
        ...
        setSensors(prev => ({
           ...prev,
           soilMoisture: last.soil,
           airTemperature: last.temp,
           airHumidity: last.humidity,
           timestamp: last.time
        }));
      }
      if (data.waterUsage) {
        setWaterUsageData(data.waterUsage);
      }
    } catch (err) {
  ...
  useEffect(() => {
    refreshData();
    const pollInterval = setInterval(refreshData, 30000); // รีเฟรชทุก 30 วินาที
  ...
  useEffect(() => {
    ...
    const runAICheck = async () => {
      // ตรวจสอบเงื่อนไข AI: ต้องเป็น Auto, ปั๊มไม่ทำงานอยู่, ไม่ได้กำลังประมวลผล และมีข้อมูลเซนเซอร์
    ...
    if (mode === SystemMode.AUTO) {
      const timer = setTimeout(runAICheck, 5000); // เริ่มเช็คครั้งแรกหลังเปิด Auto 5 วิ
      intervalId = setInterval(runAICheck, 60000); // เช็คซ้ำทุก 1 นาที
      return () => {
        clearTimeout(timer);
  ...
  const triggerPump = async (amount: number) => {
    setPumpStatus({ 
      isOn: true, 
      lastActive: new Date().toLocaleTimeString(),
      currentVolumeTriggered: amount
    });
    
    // บันทึกลง Google Sheets
    await logWaterUsageToSheet(amount);
    
    // จำลองระยะเวลาทำงานของปั๊ม (100ml ต่อ 1 วินาที)
    const duration = Math.max(2000, (amount / 100) * 1000); 
  ...
  return (
    <div className="min-h-screen bg-slate-50 bg-dot-pattern pb-20 font-sans text-slate-800 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
           ...
           <div className="flex items-center gap-2">
              ...
              <div className="hidden md:flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 ml-2">
                  ...
                  <span className="text-xs font-semibold text-slate-600">{isSyncing ? 'Syncing...' : 'Online'}</span>
              </div>
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div>
           ...
           <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            />
            <SensorCard 
              title="อุณหภูมิ" value={sensors.airTemperature} unit="°C" 
            ...
            />
            <SensorCard 
              title="ความชื้นอากาศ" value={sensors.airHumidity} unit="%" 
